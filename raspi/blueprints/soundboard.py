...
def get_sfx_folders():
    ...
    for folder in sorted(SFX_DIR.iterdir()):
        if folder.is_dir() and not folder.name.startswith('_') and folder.name != 'scripts':
...
def _play_audio_file(filepath):
    ...
    try:
        ...
        # Check for errors
        if aplay_returncode != 0:
            stderr = aplay_proc.stderr.read().decode('utf-8', errors='ignore') if aplay_proc.stderr else "No error"
            logger.error(f"aplay failed with code {aplay_returncode}: {stderr}")
            return False

        if ffmpeg_returncode != 0:
            stderr = ffmpeg_proc.stderr.read().decode('utf-8', errors='ignore') if ffmpeg_proc.stderr else "No error"
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if extensions.is_processing_job:
                    # Plotting mode
                    base_interval = extensions.audio_settings.get("plotting_frequency", 10)
                else:
                    # Idle mode
                ...
                logger.debug(f"Waiting {interval:.1f}s before next sound (base: {base_interval}s)")
