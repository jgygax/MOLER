...
ZONES = {
    ...
    "raupe": {"start": 85, "end": 101, "count": 17},
    "motherboard": {"start": 102, "end": 126, "count": 25},
}
# Gondola LED colors
...
class MultiLEDController:
    ...
    def __init__(self):
        ...
        self.raupe_position = 0.0  # Position for rotating yellow-orange LEDs
    ...
    def warm_lamp_effect(self, zone, t):
        """Schublade - Warmer pulsating lamp with left-to-right wave, stronger pulsation"""
        start, end = zone["start"], zone["end"]
        count = zone["count"]
        ...
        for i in range(count):
            ...
            # Intensity falls off with distance from wave - stronger pulsation (reduced base from 0.5)
            intensity = max(0.3, 1.0 - dist * 1.2)  # Stronger contrast

            # Small random flicker
            flicker = random.uniform(0.97, 1.0)
            intensity *= flicker

            # Much warmer, more orange tones
            r = int(255 * intensity)
            g = int(120 * intensity)  # Even more orange
            b = int(20 * intensity)  # Less blue for warmer feel

            self.set_pixel(self.strip_main, start + i, r, g, b)

    def fire_effect(self, zone, heat_array):
        """Dia-Filter - More fiery, smoother, more orange fire effect"""
        start, end = zone["start"], zone["end"]
        count = zone["count"]

        # Cool down - slower for smoother effect
        for i in range(count):
            cooldown = random.randint(0, 12)  # Even slower cooldown for smoother flames
            if cooldown > heat_array[i]:
                heat_array[i] = 0
            else:
                heat_array[i] = heat_array[i] - cooldown

        # Heat diffusion for smoother flames - more diffusion passes
        for _ in range(2):  # Two passes for extra smoothness
            for i in range(1, count - 1):
                heat_array[i] = (
                    heat_array[i - 1] + heat_array[i] + heat_array[i + 1]
                ) // 3

        # Randomly ignite new sparks - more frequent for fierier look
        if random.randint(0, 255) < 140:  # Increased from 120
            heat_array[random.randint(0, count - 1)] = random.randint(170, 240)

        # Convert heat to warm orange/yellow colors - MORE ORANGE
        for i in range(count):
            heat = heat_array[i]
            if heat > 200:  # Bright orange-yellow flames
                r = 255
                g = int(160 + (heat - 200) * 2.3)  # More yellow at high heat
                b = int(20 + (heat - 200) * 0.4)
            elif heat > 140:  # Bright orange
                r = 255
                g = int(100 + heat * 0.4)
                b = 0
            elif heat > 80:  # Orange-red
                r = int(180 + heat * 0.4)
                g = int(heat * 0.7)
                b = 0
            elif heat > 30:  # Dark orange-red
                r = int(heat * 2.5)
                g = int(heat * 0.6)  # More orange tint
                b = 0
            else:  # Deep red ember
                r = int(heat * 2.8)
                g = int(heat * 0.3)  # Slight orange even in embers
                b = 0

            self.set_pixel(self.strip_main, start + i, r, g, b)

    def laser_effect(self, zone, charge, active, cooldown):
        """Dalek Horn - Faster shooting with longer afterglow"""
        start, end = zone["start"], zone["end"]
        count = zone["count"]
        ...
        if not active:
            ...
            if cooldown <= 0 and random.random() < 0.02:  # 2% chance per frame
                return 0.0, False, 0
            ...
            # Discharge animation - faster movement
            ...
            for i in range(count):
                if i == position:
                    ...
                    r, g, b = (
                        255,
                        int(100 * discharge_progress),
                        int(100 * discharge_progress),
                    )
                elif abs(i - position) == 1:
                    # Close trail - brighter
                    trail = int(180 * discharge_progress)
                    r, g, b = trail, int(trail * 0.4), int(trail * 0.4)
                elif abs(i - position) == 2:
                    # Medium trail - new longer afterglow
                    trail = int(120 * discharge_progress)
                    r, g, b = trail, int(trail * 0.3), int(trail * 0.3)
                elif abs(i - position) == 3:
                    # Distant trail - new longer afterglow
                    trail = int(60 * discharge_progress)
                    r, g, b = trail, int(trail * 0.2), int(trail * 0.2)
                else:
                    # Faint glow everywhere
                    glow = int(20 * discharge_progress)
                    r, g, b = glow, 0, 0

                self.set_pixel(self.strip_main, start + i, r, g, b)

            charge -= 0.12  # Faster discharge (was 0.08)
    ...
    def raupe_effect(self, zone, position):
        """Raupe - 3 yellow-orange LEDs rotating in circles"""
        start, end = zone["start"], zone["end"]
        count = zone["count"]

        # Clear all LEDs in the zone first
        for i in range(start, end + 1):
            self.set_pixel(self.strip_main, i, 0, 0, 0)

        # Calculate positions of the 3 lit LEDs
        # They are evenly spaced (120 degrees apart in the circle)
        for offset in [0, count // 3, (count * 2) // 3]:
            led_pos = int((position + offset) % count)
            led_idx = start + led_pos
        ...
        # Speed: complete one rotation every ~2 seconds (count LEDs / (2 sec / DELAY))
        new_position = (position + (count / (2.0 / DELAY))) % count
        return new_position
    ...
    def circuit_effect(self, zone):
        ...
        if is_pen_down:
            ...
            self.motherboard_fill_level += (
                target_fill - self.motherboard_fill_level
            ) * 0.3
        else:
            # Pen is up - empty the bar
            if was_pen_down:
                # Just lifted pen - start emptying quickly
                self.motherboard_fill_level = max(
                    0.0, self.motherboard_fill_level - 0.15
                )
            else:
                # Continue emptying slowly
                self.motherboard_fill_level = max(
                    0.0, self.motherboard_fill_level - 0.08
                )
        ...
        for i in range(count):
            if i < progress_leds:
                ...
                i == progress_leds
                and self.motherboard_fill_level > 0
                and self.motherboard_fill_level < 1.0
            ):
    ...
    def update_gondola_led(self):
        ...
        r, g, b = (
            int(self.gondola_color[0]),
            int(self.gondola_color[1]),
            int(self.gondola_color[2]),
        )
    ...
    def _run_loop(self):
        ...
        try:
            while self.running:
                ...
                # Schublade - warmer lamp with stronger left-to-right pulsation
                self.warm_lamp_effect(ZONES["schublade"], self.time_offset)

                # Dia-filter - more fiery, smoother orange fire effect
                self.fire_effect(ZONES["dia-filter"], self.fire_heat)

                # Dalek-horn - faster laser with longer afterglow
                self.laser_charge, self.laser_active, self.laser_cooldown = (
                    self.laser_effect(
                        ZONES["dalek-horn"],
                        self.laser_charge,
                        self.laser_active,
                ...
                # Raupe - 3 yellow-orange LEDs rotating
                self.raupe_position = self.raupe_effect(
                    ZONES["raupe"], self.raupe_position
                )
